...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Check if the ProjectAggregate table is empty
        if ProjectAggregate.objects.exists():
            latest_aggregated_link_date = ProjectAggregate.objects.latest("full_date")
    ...
    def _process_collections(self, link_event_filter):
        ...
        ----------
        link_event_filter : Q
            A Q query object to filter LinkEvents by. If the ProjectAggregate table
        ...
        for collection in collections:
            ...
            for url_pattern in url_patterns:
                ...
                self._fill_project_aggregates(link_events, collection)

    def _fill_project_aggregates(self, link_events, collection):
        """
        This function loops through all link events in a URLPattern of a collection
        to check if a ProjectAggregate with prior information exists.
        If a ProjectAggregate exists, it checks if there have been any changes to the
        links added and links removed sums. If there are any changes, then the
        ProjectAggregate row is updated.
        ...
        for link_event in link_events:
            if ProjectAggregate.objects.filter(
                organisation=collection.organisation,
                collection=collection,
                project_name=link_event["domain"],
                full_date=link_event["timestamp_date"],
            ).exists():
                # Query ProjectAggregate for the existing field
                existing_link_aggregate = ProjectAggregate.objects.get(
                    organisation=collection.organisation,
                    collection=collection,
                    project_name=link_event["domain"],
                    full_date=link_event["timestamp_date"],
                ...
                ProjectAggregate.objects.create(
                    organisation=collection.organisation,
                    collection=collection,
                    project_name=link_event["domain"],
                    full_date=link_event["timestamp_date"],
