...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Check if the PageAggregate table is empty
        if PageAggregate.objects.exists():
            latest_aggregated_link_date = PageAggregate.objects.latest("full_date")
    ...
    def _process_collections(self, link_event_filter):
        ...
        ----------
        link_event_filter : Q
            A Q query object to filter LinkEvents by. If the PageAggregate table
        ...
        for collection in collections:
            ...
            for url_pattern in url_patterns:
                ...
                self._fill_page_aggregates(link_events, collection)

    def _fill_page_aggregates(self, link_events, collection):
        """
        This function loops through all link events in a URLPattern of a collection
        to check if a PageAggregate with prior information exists.
        If a PageAggregate exists, it checks if there have been any changes to the
        links added and links removed sums. If there are any changes, then the
        PageAggregate row is updated.
        ...
        for link_event in link_events:
            if PageAggregate.objects.filter(
                organisation=collection.organisation,
                collection=collection,
                page_name=link_event["page_title"],
                full_date=link_event["timestamp_date"],
            ).exists():
                # Query PageAggregate for the existing field
                existing_link_aggregate = PageAggregate.objects.get(
                    organisation=collection.organisation,
                    collection=collection,
                    page_name=link_event["page_title"],
                    full_date=link_event["timestamp_date"],
