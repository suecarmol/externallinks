...
class PageAggregatesCron(CronJobBase):
    # Will run daily at midnight UTC
    schedule = Schedule(run_at_times=["00:00"])
    code = "aggregates.page_aggregates_cron"

    def do(self):
        call_command("fill_page_aggregates")


class ProjectAggregatesCron(CronJobBase):
    # Will run daily at midnight UTC
    schedule = Schedule(run_at_times=["00:00"])
    code = "aggregates.project_aggregates_cron"
